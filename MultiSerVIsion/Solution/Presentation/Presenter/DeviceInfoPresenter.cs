...
/*
        ...
        private async void View_OnConnectClicked()
        {
            ...
            /*  if (_currentOnlineDto != null)
              ...
              // 场景2：当前选中已组态设备 → 执行正式连接
              if (!string.IsNullOrWhiteSpace(*//*_currentConfigDeviceId*//* deviceId))
              {
                  await DoFormalConnect(deviceId);
              }*/
        }
       /* private async Task DoConnectTest()
        {
            _view.UpdateConnectStatus(CameraStatus.Connected);
            ...
            try
            {
                ...
                var testResult = await _cameraAppService.TestConnectAsync(connectConfig);
                
                if (testResult.Success)
                {
                    _view.ShowMessage("连接测试成功，设备可正常运行");
                }
                else
                {
                    _view.ShowMessage($"连接测试失败{testResult.Message}");
                }
                _view.UpdateConnectStatus(CameraStatus.Disconnected);
            ...
            {
                _view.UpdateConnectStatus(CameraStatus.Disconnected);
                _view.ShowMessage($"连接测试异常{ex.Message}");
            }
        }*/

        private async Task DoFormalConnect(string device)
        {
            _view.UpdateConnectStatus(CameraStatus.Connected);
            try
            {
                var result = await _cameraAppService.ConnectCamera(device);
                if(result.Success)
                {
                    _deviceContext.UpdateConnectionStatus(_currentOnlineDto.Model, true);
                  /*  _eventBus.Publish(new DeviceConnectionChangedEvent(deviceId, true));*/
                    // 3. 更新自身视图
                    _view.UpdateConnectStatus(CameraStatus.Connected);
                    _currentStatus = CameraStatus.Connected;
                    _view.ShowMessage("设备连接成功");
                }
                else
                {
                    _view.ShowMessage($"连接失败：{result.Message}");
            ...
            catch (Exception ex)
            {
                _currentStatus = CameraStatus.Disconnected;
                _view.UpdateConnectStatus(_currentStatus);
                _view.ShowMessage($"连接异常：{ex.Message}");
            }
        }
